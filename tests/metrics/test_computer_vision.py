...
class TestIOU(unittest.TestCase):
   ...
   def test_average_precision_single(self):
      for i in range(len(GT)):
         ...
         self.assertTrue(torch.isclose(ap_i, torch.tensor(AP[i]), atol=1e-4), (ap_i, AP[i]))
   ...
   def test_average_precision_batched(self):
      ...
      self.assertTrue(torch.allclose(overall_ap, torch.tensor(AP_overall), atol=1e-4), (overall_ap, AP_overall))
